...
public class PrestamoUno extends PrestamoPrueba implements MetodoFrances {
    ...
    public double calcularCuotaFija() {
        double cuota = calcularSeguroIncendios()
                + (saldoFinal * INTERES_ANUAL) / (1 - Math.pow(1 + INTERES_ANUAL, -getMeses()));
        // Si se requieren los seguros en la cuota, se pueden sumar aquí (comentado por
        // ahora)
        // cuota += calcularSeguroIncendios() + calcularSeguroDesgravamen();
        return cuota;
    }

    // 2. Método para calcular el interés del período actual usando el saldo
    // pendiente
